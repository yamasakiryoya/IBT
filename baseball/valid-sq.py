...
if os.path.isdir("Results-sq2/%f"%r)==False: os.makedirs("Results-sq2/%f"%r, exist_ok=True)
if os.path.exists("Results-sq2/%f/error-%f-%d.csv"%(r,r,seed))==False:
    rd.seed(seed)
    # data
    ...
    train_idx_list = ind[:split_idx]
    train_ij = []
    for k in train_idx_list:
        train_ij.append(all_ij[k])
    #
    ITE = 10
    FOLD = 10
    err = np.zeros((FOLD,ITE,32))
    for K in range(FOLD):
        ind = np.arange(len(train_ij));
        np.random.shuffle(ind)
        split_idx = int(len(train_ij)/FOLD)
        train_idx_list = ind[split_idx:]; test_idx_list = ind[:split_idx]
        train_P = np.zeros((n, n)); test_P = np.zeros((n, n))
        for idx in train_idx_list:
            i, j = train_ij[idx]
            train_P[i,j] = 1; train_P[j,i] = 1
        ...
        for ite in range(ITE):
            ...
            # if ite!=0 and res[t,-2]>err[K,ite,10]: break
            ...
            # evaluation
            if ite==0:
                err[K,ite,0] = obj(est[ite], W, train_P)
                err[K,ite,1] = obj(est[ite], W, test_P)
                err[K,ite,2] = obj(est[ite], Y, train_P)
                err[K,ite,3] = obj(est[ite], Y, test_P)
                err[K,ite,4] = rnk1(est[ite], W, train_P)
                err[K,ite,5] = rnk1(est[ite], W, test_P)
                err[K,ite,6] = rnk2(est[ite], W, train_P)
                err[K,ite,7] = rnk2(est[ite], W, test_P)
                err[K,ite,8] = rnk3(est[ite], W, train_P)
                err[K,ite,9] = rnk3(est[ite], W, test_P)
                err[K,ite,10]= rnk4(est[ite], W, train_P)
                err[K,ite,11]= rnk4(est[ite], W, test_P)
                err[K,ite,12]= rnk5(est[ite], W, train_P)
                err[K,ite,13]= rnk5(est[ite], W, test_P)
                err[K,ite,14]= rnk6(est[ite], W, train_P)
                err[K,ite,15]= rnk6(est[ite], W, test_P)
            else:
                err[K,ite,0] = iso_obj(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,1] = iso_obj(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,2] = iso_obj(est[ite], Y, train_P, old_PX, old_PY)
                err[K,ite,3] = iso_obj(est[ite], Y, test_P,  old_PX, old_PY)
                err[K,ite,4] = iso_rnk1(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,5] = iso_rnk1(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,6] = iso_rnk2(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,7] = iso_rnk2(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,8] = iso_rnk3(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,9] = iso_rnk3(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,10]= iso_rnk4(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,11]= iso_rnk4(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,12]= iso_rnk5(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,13]= iso_rnk5(est[ite], W, test_P,  old_PX, old_PY)
                err[K,ite,14]= iso_rnk6(est[ite], W, train_P, old_PX, old_PY)
                err[K,ite,15]= iso_rnk6(est[ite], W, test_P,  old_PX, old_PY)
            err[K,ite,16] = iso_obj(est[ite], W, train_P, PX, PY)
            err[K,ite,17] = iso_obj(est[ite], W, test_P,  PX, PY)
            err[K,ite,18] = iso_obj(est[ite], Y, train_P, PX, PY)
            err[K,ite,19] = iso_obj(est[ite], Y, test_P,  PX, PY)
            err[K,ite,20] = iso_rnk1(est[ite], W, train_P, PX, PY)
            err[K,ite,21] = iso_rnk1(est[ite], W, test_P,  PX, PY)
            err[K,ite,22] = iso_rnk2(est[ite], W, train_P, PX, PY)
            err[K,ite,23] = iso_rnk2(est[ite], W, test_P,  PX, PY)
            err[K,ite,24] = iso_rnk3(est[ite], W, train_P, PX, PY)
            err[K,ite,25] = iso_rnk3(est[ite], W, test_P,  PX, PY)
            err[K,ite,26] = iso_rnk4(est[ite], W, train_P, PX, PY)
            err[K,ite,27] = iso_rnk4(est[ite], W, test_P,  PX, PY)
            err[K,ite,28] = iso_rnk5(est[ite], W, train_P, PX, PY)
            err[K,ite,29] = iso_rnk5(est[ite], W, test_P,  PX, PY)
            err[K,ite,30] = iso_rnk6(est[ite], W, train_P, PX, PY)
            err[K,ite,31] = iso_rnk6(est[ite], W, test_P,  PX, PY)
    for K in range(FOLD):
        for ite in range(ITE):
            if np.all(err[K,ite,:] == 0):
                err[K,ite,:16] = err[K,ite-1,16:]
                err[K,ite,16:] = err[K,ite-1,16:]
    error = np.mean(err, axis=0)
    np.savetxt("Results-sq2/%f/error-%f-%d.csv"%(r,r,seed), error, delimiter=",")
