...
for ite in range(ITE):
    ...
    # evaluation
    Q = est[ite].reshape(-1,1)-est[ite].reshape(1,-1)
    if ite==0:
        err[0] = rankdata(np.sum(expit(Q), axis=0), method='min')
        M = model(Q, PX, PY)
        err[1] = rankdata(np.sum(M, axis=0), method='min')
    else:
        old_M = model(Q, old_PX, old_PY)
        err[2*ite] = rankdata(np.sum(old_M, axis=0), method='min')
        M = model(Q, PX, PY)
        err[2*ite+1] = rankdata(np.sum(M, axis=0), method='min')

np.savetxt("Results-nll.csv", err, delimiter=",", fmt='%d')