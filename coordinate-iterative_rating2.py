...
for n in [1000]:# number of subjects
    for m in [100]:# number of problems
        ...
        def true_obj(A, B, Y, P):
            ...
            return tmp/len(P)
        print("true\ttrain:",true_obj(A, B, Y, train_P))
        ...
        def objA(A, B, Y, P,k):
            C = A[k]-B
            tmp = np.sum(-log_expit(-C[Y[k,:]==0.]))+np.sum(-log_expit(C[Y[k,:]==1.]))
            #tmp = np.sum(np.square(Y[k,:]-expit(C)))
            return tmp
        def objB(A, B, Y, P,k):
            C = A-B[k]
            tmp = np.sum(-log_expit(-C[Y[:,k]==0.]))+np.sum(-log_expit(C[Y[:,k]==1.]))
        ...
        def gradB(A, B, Y, P, k):
            ...
            """
            D = expit(C); E = expit(-C)
            tmpB = np.sum((Y[:,k]-D)*D*E)
            """
            return tmpB


        ITE = 10; estA = np.zeros((ITE,n)); estB = np.zeros((ITE,m)); estO = np.zeros(ITE)
        for ite in range(ITE):
            EP1 = 30; EP2 = 100; resA = np.zeros((EP1,n)); resB = np.zeros((EP1,m)); resO = np.zeros(EP1)
            # epoch 0
            if ite==0:
                resO[0] = obj(resA[0], resB[0], Y, train_P)
            if ite!=0:
                old_PX = PX.copy(); old_PY = PY.copy()
                resA[0] = estA[ite-1].copy(); resB[0] = estB[ite-1].copy()
                resO[0] = iso_obj(resA[0], resB[0], Y, train_P, old_PX, old_PY)
            ...
            for t in range(1,EP1):
                for k in range(n+m):
                    #
                    if k<n:
                        if ite==0: resO_old = objA(resA_old, resB_old, Y, train_P, k)
                        if ite!=0: resO_old = iso_objA(resA_old, resB_old, Y, train_P, old_PX, old_PY, k)
                    else:
                        if ite==0: resO_old = objB(resA_old, resB_old, Y, train_P, k-n)
                        if ite!=0: resO_old = iso_objB(resA_old, resB_old, Y, train_P, old_PX, old_PY, k-n)
                    #print("iteration:",ite+1,"\tepoch:",t+1,"\tinner epoch:",k+1,0,"\ttrain:",resO_old)#if s!=0: 
                    for s in range(EP2):
                        if k<n:
                            if ite==0: tmpA = gradA(resA_old, resB_old, Y, train_P, k)
                            if ite!=0: tmpA = iso_gradA(resA_old, resB_old, Y, train_P, old_PX, old_PY, k)
                            LR = 1./np.max([.1**30,np.abs(tmpA)])#if s%3==0: 
                        else:
                            if ite==0: tmpB = gradB(resA_old, resB_old, Y, train_P, k-n)
                            if ite!=0: tmpB = iso_gradB(resA_old, resB_old, Y, train_P, old_PX, old_PY, k-n)
                            LR = 1./np.max([.1**30,np.abs(tmpB)])#if s%3==0: 
                        #
                        flag1 = 0
                        while flag1 == 0:
                            if k<n:
                                resA_new[k] = resA_old[k] - LR*tmpA
                                if ite==0: resO_new = objA(resA_new, resB_new, Y, train_P, k)
                                if ite!=0: resO_new = iso_objA(resA_new, resB_new, Y, train_P, old_PX, old_PY, k)
                            else:
                                resB_new[k-n] = resB_old[k-n] - LR*tmpB
                                if ite==0: resO_new = objB(resA_new, resB_new, Y, train_P, k-n)
                                if ite!=0: resO_new = iso_objB(resA_new, resB_new, Y, train_P, old_PX, old_PY, k-n)
                            #
                            if LR<.1**30:#(k<n and tmpA==0.) or (k>=n and tmpB==0.) or 
                                flag1 = 2
                ...
                if ite==0: resO[t] = obj(resA[t], resB[t], Y, train_P)
                if ite!=0: resO[t] = iso_obj(resA[t], resB[t], Y, train_P, old_PX, old_PY)
                #print("iteration:",ite+1,"\tepoch:",t+1,"\ttrain:",resO[t])
                if resO[t]==resO[t-1]: break
                else: pass
            final_t = t+1
            estA[ite] = resA[t].copy(); estB[ite] = resB[t].copy(); estO[ite] = resO[t].copy()
            if estO[ite]==estO[ite-1]: break


            # data preparation
            L = len(train_P); Xij = np.zeros(L); Yij = np.zeros(L)
            for t, (i,j) in enumerate(train_P): Xij[t] = estA[ite,i]-estB[ite,j]; Yij[t] = Y[i,j]
            ...
            if ite==0: a = obj(estA[ite], estB[ite], Y, train_P)
            else: a = iso_obj(estA[ite], estB[ite], Y, train_P, old_PX, old_PY)
            c = iso_obj(estA[ite], estB[ite], Y, train_P, PX, PY)
            print("train:",a,c)
            ...
            plt.savefig("./2-%d-%d-%d-1-A.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
            #
            ...
            ax.set_xlabel(r'epoch'); ax.grid(True); plt.tight_layout()
            plt.savefig("./2-%d-%d-%d-1-B.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
            ...
            if ite==0:
                ...
                ax.plot(FX, model(FX, PX, PY), lw=2, linestyle="--", color="limegreen", zorder=3, label=r"prediction $\hat{\sigma}^{[1]}(\hat{a}_i^{[1]}-\hat{b}_j^{[1]})$")#"isotonic rating")
            else:
                ...
                ax.plot(FX, model(FX, PX, PY), lw=2, linestyle="--", color="limegreen", zorder=3, label=r"prediction $\hat{\sigma}^{[%d]}(\hat{a}_i^{[%d]}-\hat{b}_j^{[%d]})$"%(ite+1,ite+1,ite+1))#"isotonic rating @ iter.=%d"%(ite+1))
            #
            ...
            plt.savefig("./2-%d-%d-%d-2.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
            ...
            MORC = 0
            for t in range(n*m):
                if (0.2+0.8*(np.arctan(train_E[t])/np.pi+0.5)<0.5 and model(train_G[t], PX, PY)>=0.5) or (0.2+0.8*(np.arctan(train_E[t])/np.pi+0.5)>0.5 and model(train_G[t], PX, PY)<=0.5): MORC += 1
            print("A: %d/%d=%f"%(MORA,n*(n-1),MORA/n/(n-1)))
            print("B: %d/%d=%f"%(MORB,m*(m-1),MORB/m/(m-1)))
            print("C: %d/%d=%f"%(MORC,n*m,MORC/n/m))
            ...
            ax.set_title(r"${\rm misordering}=\frac{%d}{%d}$"%(MORC,n*m))
            ax.set_xlim(-maxijG*1.1,maxijG*1.1); ax.set_ylim(-maxijE*1.1,maxijE*1.1); ax.grid(True); plt.tight_layout()
            plt.savefig("./2-%d-%d-%d-3.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
            ...
            plt.savefig("./2-%d-%d-%d-4-A.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
            #
            fig = plt.figure(figsize=(6,6)); cmap = plt.get_cmap('jet'); ax = fig.add_subplot(111)
            ...
            plt.savefig("./2-%d-%d-%d-4-B.png"%(n,m,ite), bbox_inches="tight", pad_inches=.02, dpi=200, facecolor=fig.get_facecolor(), edgecolor='none'); plt.close()
